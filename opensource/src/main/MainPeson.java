...
public class MainPeson {
		public static void main(String[] args) {
			while(true) {
				...
				System.out.println("3 : hien thi thong tin ve xe");
		...
		public static void info(Person p ) {
			if(p instanceof car) {
				...
				System.out.println("gia ve xe " + ((car)p).getPrice() + "cho ngoi " + ((car)p).getSlot() + "loai xe " + ((car)p).getTyle());
			}else if(p instanceof Train) {
				
				System.out.println("gia ve xe " + ((Train)p).getPrice() + " cho ngoi "+ ((Train)p).getSlot() + "so toa " + ((Train)p).getSotoa() );
			}
		}
}
